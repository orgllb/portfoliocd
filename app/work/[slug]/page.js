  ...
  if (!project) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p style={{ color: '#555550' }}>Project not found</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-8 py-12">
        <Link href="/work" style={{ color: '#444440', fontSize: '10px' }} className="uppercase mb-12 inline-block hover:opacity-70 transition-opacity">
          Back to Work
        </Link>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mb-16">
          <div>
            <p className="text-[10px] text-[#444440] uppercase mb-4">{project.category} - {project.year}</p>
            <h1 style={{ fontFamily: 'var(--font-display)' }} className="text-[5rem] text-[#EBEBEB] leading-none mb-8">{project.title}</h1>
            <p className="text-[11px] text-[#666660] leading-loose mb-8">{project.description}</p>
            <div className="flex flex-wrap gap-2">
              {project.tags.map((tag) => (
                <span key={tag} className="text-[10px] text-[#555550] border border-[#222220] px-3 py-1 uppercase">{tag}</span>
              ))}
            </div>
          </div>
        ...
        <div className="border-t border-[#1a1a18] pt-10">
          <Link href="/work" style={{ color: '#444440', fontSize: '11px' }} className="uppercase hover:opacity-70 transition-opacity">
            Back to all work
          </Link>
        </div>
      </main>
