  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-8 py-12">
        <p className="text-[10px] text-[#999992] uppercase mb-4">Journal</p>
          ...
          {posts.map((post) => (
            <li key={post.id} className="border-b border-[#1a1a18] py-6">
              <Link href={'/blog/' + post.slug} className="block hover:opacity-70 transition-opacity">
                ...
                <p className="text-[10px] text-[#8a8a84]">{post.date}</p>
              </Link>
            </li>
          ))}
        </ul>
        <p className="text-[10px] text-[#8a8a84] uppercase mb-3">
  Inspiration
</p>
<p className="text-[11px] text-[#a8a8a1] mb-6">
