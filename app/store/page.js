  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-8 py-12">
        <p className="text-[12px] text-[#999992] uppercase mb-4">Peakd Shop</p>
        <h1 style={{ fontFamily: 'var(--font-display)' }} className="text-[6rem] text-[#EBEBEB] leading-none mb-6">
          Shop
        </h1>
        <p className="text-[13px] text-[#a8a8a1] leading-loose mb-16 max-w-xl">
          A rotating edit of Peakd essentials. Limited drops, no restocks.
        </p>

        {loading && (
          <p className="text-[13px] text-[#a8a8a1] mb-8">Loading products...</p>
        )}

        {error && (
          <p className="text-[13px] text-[#a8a8a1] mb-8">Could not load products: {error}</p>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-[#1a1a18]">
          {products.map((product) => (
            <Link key={product.id} href={'/store/' + product.id} className="group bg-[#111111] p-6 hover:bg-[#161614] transition-colors block">
              ...
              <p className="text-[12px] text-[#8a8a84] uppercase mb-2">{product.category}</p>
