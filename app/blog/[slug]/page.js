  ...
  if (!post) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p style={{ color: '#a8a8a1' }}>Post not found</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-8 py-12">
        <Link href="/blog" style={{ color: '#999992', fontSize: '10px' }} className="uppercase mb-12 inline-block hover:opacity-70 transition-opacity">
          Back to Journal
        </Link>
        <div className="max-w-2xl mt-8">
          ...
          {post.body.map((para, i) => (
            <p key={i} className="text-[12px] text-[#b8b8b1] leading-loose mb-6">{para}</p>
          ))}
        </div>
        <div className="border-t border-[#1a1a18] pt-10 mt-16">
          <Link href="/blog" style={{ color: '#999992', fontSize: '11px' }} className="uppercase hover:opacity-70 transition-opacity">
            Back to Journal
          </Link>
        </div>
      </main>
